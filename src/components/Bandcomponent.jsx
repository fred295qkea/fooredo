...
export default function Bandcomponent({ data, isFavorite, toggleFavorite }) {
  ...
  return (
    <div className={`m-2 py-2 px-3 flex justify-between ${backgroundColor}`}>
      ...
      <button onClick={() => toggleFavorite(data.name)}>{isFavorite ? <span className="material-icons icon">favorite</span> : <span className="material-icons icon">favorite_border</span>}</button>
    </div>
  );
}
